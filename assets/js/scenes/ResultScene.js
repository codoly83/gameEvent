...
젊고 튼튼한지 체크해 보세요!",
  ...
  create() {
    ...
    const closeBtn = this.add
      .sprite(this.sys.game.config.width - 20, 20, "close")
      .setInteractive({ cursor: "pointer" })
      .setDepth(2000)
      .setOrigin(1, 0); //this.add.text(20,20,'닫기', { color: '#000', font: '700 40px DOSIyagi' }).setInteractive({ cursor: 'pointer' });
    ...
    this.popupBack.fillRoundedRect(
      -this.sys.game.config.width / 2 + 100,
      -this.sys.game.config.height / 2 + 560 - offset,
      874,
      1148 + offset,
      48,
    );

    this.popup.title = this.add
      .text(0, -280, "순발력으로 보는\r\n나의 신체나이는?", {
        align: "center",
        font: "bold 72px DOSIyagi",
        lineSpacing: 8,
    ...
    this.popup.age = this.add
      .text(0, -80, age, {
        align: "center",
        font: "bold 90px DOSIyagi",
        lineSpacing: 8,
    ...
    //this.popup.content = this.add.text(0,  - 35, content, {align: 'center', fontFamily: 'DOSIyagi', fontSize: '46px', color: '#000000', wordWrap: { width: 600, useAdvancedWrap: true } }).setOrigin(0.5);

    this.popup.content = this.createAutoResizeText(
      0,
      175,
      content,
      {
        align: "center",
        font: "500 45px DOSIyagi",
        lineSpacing: 6,
        color: "#000000",
      },
      46,
    ...
    this.popup.startBtn = this.add
      .image(0, 0, "go_btn")
      .setOrigin(0.5)
      .setInteractive({ cursor: "pointer" });
    this.popup.replayBtn = this.add
      .image(0, 624, "replay_btn")
      .setOrigin(0.5)
      .setInteractive({ cursor: "pointer" });
    ...
    this.popup.deco = this.add.container(
      0,
      -this.sys.game.config.height / 2 + 480 - offset + 160,
    );
    ...
    for (let i = 0; i < textures.length; i++) {
      const emitter1 = this.add.particles(
        ...
        "particle" + (i + 1),
        {
          lifespan: 12000,
          x: {
            min: -this.sys.game.config.width / 2,
            max: this.sys.game.config.width,
          },
          speed: 100,
          scale: { min: 0.9, max: 1.3 },
          angle: { min: 0, max: 360 },
          rotate: { min: 0, max: 360 }, // 👈 랜덤 회전
          gravityY: 120,
          frequency: 1200,
          emitting: false,
        },
  ...
  createAutoResizeText(x, y, content, style, fontSize, maxWidth, maxHeight) {
    const text = this.add
      .text(
        ...
        y,
        content,
        Object.assign(style, { wordWrap: { width: maxWidth } }),
      )
      .setOrigin(0.5);

    while (text.height > maxHeight && fontSize > 10) {
      // 최소 폰트 10px
      fontSize--;
